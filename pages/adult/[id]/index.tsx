...
//   const title = item.title || item.name || 'Adult Movies';
//   const ytId = item.yt_id;
//   const canonicalUrl = `${BASE_URL}/adult/${item.id}`;
...
//             </div>
//           </div>
//           <div className="mt-12"><Recommendations items={recommendations} basePath="/adult" title="More Adult Movies" /></div>
//         </main>
//         <Footer />
//       </div>
//     </>
...
//   paths: UNIQUE_ADULT.map((item) => ({ params: { id: String(item.id) } })),
//   fallback: 'blocking',
// });
...
//   try {
//     const movie = UNIQUE_ADULT.find((m) => String(m.id) === id);
...
//     const recommendations = UNIQUE_ADULT.filter((m) => String(m.id) !== id).slice(0, 6).map((m) => sanitizeMediaItem(m));
//     return { props: { item: sanitizedItem, recommendations, ogImage }, revalidate: 3600 };
//   } catch (error) {
//     console.error(`Error in getStaticProps for adult ${id}:`, error);
