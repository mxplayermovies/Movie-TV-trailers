...
//   const description = item.overview?.slice(0, 160) || `Watch ${title} online in HD on Movie & TV trailers. 18+ content.`;
...
//     const recommendations = UNIQUE_ADULT
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a]">
        ...
        <main className="container mx-auto px-4 py-8">
          ...
          <div className="mt-12">
            <Recommendations items={recommendations} basePath="/adult" title="More Movies" />
          </div>
        </main>
        <Footer />
      </div>
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  try {
    ...
    const imagePath = sanitizedItem.backdrop_path || sanitizedItem.poster_path;
    if (!imagePath) {
      console.error(`adult "${sanitizedItem.title}" (id: ${id}) has no image.`);
      return { notFound: true };
    }
