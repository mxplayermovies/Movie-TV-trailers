...
//       voiceManager.playBackgroundMusic(DEFAULT_BG_MUSIC_IDS.adult);
//       setTimeout(() => {
//         if (cancelled) return;
//         try {
//           voiceManager.speak(getPageText() || 'Adult movies page. Browse the latest adult movie collection.', true, true);
//         } catch { }
//       }, 1500);
//     }).catch(() => {});
//     return () => {
...
//   const readPageContent = () => { try { voiceRef.current?.speak(getPageText() || 'Adult movies page.', true, true); } catch { } };
//   const loadMore = () => setVisibleCount(prev => Math.min(prev + ITEMS_PER_PAGE, totalItems));
//   const visibleItems = items.slice(0, visibleCount);
...
//           <div className="flex justify-between items-center mb-8">
//             <h1 className="text-3xl md:text-4xl font-bold">Adult Movies</h1>
//             <p className="page-text hidden">We have {items.length} Hot Adult Movies available. Updated as on {buildDateString}. We update our content regularly to ensure you have the latest and greatest Adult movie trailers and information every 24 hours. Watch our Latest Adult movie library and enjoy the best free streaming experience.</p>
//             <div className="flex items-center gap-2">
//               <span className="text-sm text-gray-500 dark:text-gray-300">Read details aloud</span>
//               <button onClick={readPageContent} className="p-2 bg-blue-500/20 text-blue-400 rounded-full hover:bg-blue-500 hover:text-white transition" aria-label="Read page aloud"><Volume2 size={20} /></button>
//             </div>
...
export default function AdultPage({ items, buildDateString }: Props) {
  ...
  const readPageContent = () => { 
    if (isMuted) return;
    try { voiceRef.current?.speak(getPageText() || 'Adult movies page.', true, true); } catch { } 
  };
  const loadMore = () => setVisibleCount(prev => Math.min(prev + ITEMS_PER_PAGE, totalItems));
  const visibleItems = items.slice(0, visibleCount);
