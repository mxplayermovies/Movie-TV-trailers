...
// import { UNIQUE_DOCUMENTARY, getDetails, getImageUrl } from '../../../services/tmdb';
...
// export default function DocumentaryDetail({ item, recommendations, ogImage }: Props) {
...
//   const description = item.overview?.slice(0, 160) || `Watch ${title} online in HD on Movie & TV trailers.`;
...
//   // Documentary is still a Movie in schema.org terms
//   const movieSchema = {
...
//     "contentRating": "PG", // Documentaries often PG
...
//     const recommendations = UNIQUE_DOCUMENTARY
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  try {
    ...
    const imagePath = sanitizedItem.backdrop_path || sanitizedItem.poster_path;
    if (!imagePath) {
      console.error(`documentary "${sanitizedItem.title}" (id: ${id}) has no image.`);
      return { notFound: true };
    }
