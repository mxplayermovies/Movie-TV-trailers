...
// import { UNIQUE_DOCUMENTARY, getDetails } from '../../../services/tmdb';
...
// export default function DocumentaryDetail({ item, recommendations }: Props) {
...
//   useEffect(() => {
//     if (title) {
//       voiceManager.speak(`Now viewing ${title}. Click the speaker icon to learn about the documentary movie.`);
//     }
//   }, [title]);
...
//         <meta property="og:url" content={`https://movie-tv-trailers.vercel.app/documentary/${item.id}`} />
...
//                 duration: item.duration || 'PT1H30M',
...
//           <Recommendations items={recommendations} basePath="/documentary" title="More Documentaries" />
//         </main>
//         <Footer />
//       </div>
//     </>
...
//   const paths = UNIQUE_DOCUMENTARY.map((item) => ({
...
//     const allItems = UNIQUE_DOCUMENTARY.map(sanitizeMediaItem);
...
//   const title = item.title || item.name || 'Documentary';
//   const ytId = item.yt_id;
//   const shareUrl = `${BASE_URL}/documentary/${item.id}`;
...
// import { GetStaticPaths, GetStaticProps } from 'next';
// import { useRouter } from 'next/router';
// import { UNIQUE_TV_SHOWS, getDetails } from '../../../services/tmdb';
// import { ContentDetails, MediaItem } from '../../../types';
// import Header from '../../../components/Header';
// import Footer from '../../../components/Footer';
// import YouTubePlayer from '../../../components/YouTubePlayer';
...
// export default function TvShowDetail({ item, recommendations }: Props) {
//   const router = useRouter();
//   const [loading, setLoading] = useState(false);
...
//   useEffect(() => {
//     if (title) {
//       voiceManager.speak(`Now viewing ${title}. Click the speaker icon to learn more about the TV series.`);
//     }
//   }, [title]);
...
//         <meta property="og:description" content={description} />
//         <meta property="og:image" content={item.poster_path || '/og-image.jpg'} />
//         <meta property="og:url" content={`https://movie-tv-trailers.vercel.app/tv/${item.id}`} />
//         {youtubeWatchUrl && <meta property="og:video" content={youtubeWatchUrl} />}
//         {youtubeEmbedUrl && <meta property="og:video:url" content={youtubeEmbedUrl} />}
//         <meta property="og:video:type" content="text/html" />
//         <meta property="og:video:width" content="1280" />
...
// export const getStaticPaths: GetStaticPaths = async () => {
//   const paths = UNIQUE_TV_SHOWS.map((item) => ({
//     params: { id: String(item.id) },
//   }));
//   return { paths, fallback: 'blocking' };
// };
...
export default function TvShowDetail({ item, recommendations }: Props) {
  ...
  const title = item.title || item.name || 'TV Show';
  const ytId = item.yt_id;
  const shareUrl = `${BASE_URL}/tv/${item.id}`;
