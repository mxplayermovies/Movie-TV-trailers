...
// import { Tv, Search, X, Volume2, VolumeX } from 'lucide-react';
...
//   // Background music ID for TV shows page
//   const getBgMusicId = useCallback((): string | null => {
//     return DEFAULT_BG_MUSIC_IDS.tv || DEFAULT_BG_MUSIC_IDS.home || DEFAULT_BG_MUSIC_IDS.movies;
//   }, []);
...
//       } else {
//         console.warn('No background music ID for TV shows page');
//       }
...
//   // ---------- Fetch latest TV shows from current year ----------
...
//     "description": "Watch the newest TV series from this year on Movie & TV trailers.",
...
//       <SEO 
//         title="TV Shows - Movie & TV trailers" 
...
// pages/tv.tsx
import React, { useEffect, useState, useCallback } from 'react';
import { getTVShows, searchContent, UNIQUE_TV_SHOWS } from '../services/tmdb';
import MovieCard from '../components/MovieCard';
import SEO from '../components/SEO';
import { TVShow, MediaItem } from '../types';
import { Tv, Search, X, Volume2, VolumeX } from 'lucide-react';
 ...
 * significantly longer than the English originals. The helpers below defend
 * against overflow at every text node.
 *
 *  TEXT_SAFE  – applied to every element that renders text
 *  fluidFont  – CSS clamp()-based size that interpolates between minPx @320 vw
 *               and maxPx @1280 vw so text scales proportionally on any device
 * ─────────────────────────────────────────────────────────────────────────── */
...
/**
 * Merges UNIQUE_TV_SHOWS (static custom items) with the API results and
 * shuffles the combined list so custom shows surface randomly throughout
 * the grid rather than all appearing at the top or bottom.
 *
 * Deduplication is done by `id` so a show that exists in both sources is
 * only shown once (the static version wins so streams are always present).
 */
function mergeAndShuffle(apiShows: TVShow[], customShows: MediaItem[]): MediaItem[] {
  const seenIds = new Set<string | number>();
  const merged: MediaItem[] = [];

  // Custom shows win on conflict – add them first into the dedup set
  for (const show of customShows) {
    seenIds.add(String(show.id));
    merged.push(show as MediaItem);
  }

  // Add API shows only if not already represented
  for (const show of apiShows) {
    if (!seenIds.has(String(show.id))) {
      seenIds.add(String(show.id));
      merged.push(show as unknown as MediaItem);
    }
  }
...
const TVShows: React.FC = () => {
  ...
  // ── Fetch latest TV shows from current year (with UNIQUE_TV_SHOWS merged) ──
  ...
  // ── Initial load – get current year shows merged with UNIQUE_TV_SHOWS ────

  useEffect(() => {
    const fetchShows = async () => {
      setLoading(true);
      try {
        const apiData  = await getCurrentYearTVShows(1);
        const filtered = apiData.filter((show: TVShow) => show.poster_path);

        // Merge API results with ALL UNIQUE_TV_SHOWS and shuffle together
        const merged = mergeAndShuffle(filtered, UNIQUE_TV_SHOWS);
        setShows(merged);
        setPage(1);
      } catch (error) {
        console.error(error);
        // Fallback: just show the custom shows shuffled
        setShows(shuffleArray([...UNIQUE_TV_SHOWS]));
      } finally {
        setLoading(false);
      }
    };
  ...
  // ── Load more – appends next API page shuffled with remaining custom shows ─
  // (custom shows are already in the list so we just append fresh API pages)
  ...
  const loadMore = async () => {
    ...
    try {
      const nextPage = page + 1;
      const data     = await getCurrentYearTVShows(nextPage);
      const filtered = data.filter((show: TVShow) => show.poster_path);

      // De-duplicate against already-shown ids before appending
      const existingIds = new Set(shows.map((s) => String(s.id)));
      const newShows    = shuffleArray(
        filtered.filter((s: TVShow) => !existingIds.has(String(s.id))) as unknown as MediaItem[]
      );
      ...
      >
        <div style={{ maxWidth: '56rem', margin: '0 auto' }}>
          <form onSubmit={handleSearch} className="relative">
            ...
            >
              <Search className="text-gray-400 shrink-0" size={20} style={{ marginLeft: 'clamp(8px,2vw,16px)' }} />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
      ...
      >
        ...
        >
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <p
              ...
              style={{ ...fluidFont(12, 14), ...TEXT_SAFE }}
            >
              Newest series from {new Date().getFullYear()} · Including exclusive web series
            </p>
          </div>
        </div>

        {/* Shows grid — custom UNIQUE_TV_SHOWS are shuffled into API results */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4 md:gap-6">
          {shows.map((show) => (
            <MovieCard key={String(show.id)} item={show as any} />
          ))}
        </div>
        ...
        {hasMore && (
          ...
          >
            ...
            >
              <span style={TEXT_SAFE}>{loadingMore ? 'Loading...' : 'Load More Shows'}</span>
            </button>
          </div>
        )}
      </div>
