...
import { getLiveTV, searchContent } from '../services/tmdb';
import MovieCard from '../components/MovieCard';
import SEO from '../components/SEO';
import { MediaItem } from '../types';
import { Radio, ArrowDownCircle, Search, X, Volume2, VolumeX } from 'lucide-react';
...
const LiveTV: React.FC = () => {
  ...
  // Background music ID for live TV page — correct key is 'tv-live'
  const getBgMusicId = useCallback((): string | null => {
    return DEFAULT_BG_MUSIC_IDS['tv-live'] || DEFAULT_BG_MUSIC_IDS.home || DEFAULT_BG_MUSIC_IDS.movies;
  }, []);
  ...
  const handleSpeakerClick = () => {
    if (isSpeaking) {
      ...
      if (videoId) {
        ...
        console.warn('No background music ID for live TV page');
      }
  ...
  const schema = {
    ...
    "mainEntity": {
      ...
      "itemListElement": channels.slice(0, 10).map((item, index) => ({
        "@type": "ListItem",
        "position": index + 1,
        "url": `https://movie-tv-trailers.vercel.app/watch/tv_live/${item.id}`,
  ...
  return (
    <div className="min-h-screen bg-miraj-black pt-24 pb-20 relative">
      ...
      <SEO
        title="Live TV - Watch News & Entertainment | Movie & TV trailers"
