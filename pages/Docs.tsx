...
import { UNIQUE_DOCUMENTARY, searchContent } from '../services/tmdb';
...
const Documentary: React.FC = () => {
  ...
  // Background music ID for documentary page
  const getBgMusicId = useCallback((): string | null => {
    return DEFAULT_BG_MUSIC_IDS.documentary || DEFAULT_BG_MUSIC_IDS.movies || DEFAULT_BG_MUSIC_IDS.home;
  }, []);
  ...
  // Build a spoken version of the Documentary page
  const buildSpeakText = (): string => {
    const intro = "Documentaries section. ";
    const sections = [
  ...
  const handleSpeakerClick = () => {
    if (isSpeaking) {
      ...
      if (videoId) {
        ...
        console.warn('No background music ID for documentary page');
      }
  ...
  const schema = {
    ...
    "@type": "CollectionPage",
    "name": "Documentaries - Movie & TV trailers",
    "description": "Watch the best documentary films free in HD on Movie & TV trailers.",
    "url": "https://movie-tv-trailers.vercel.app/Docs"
  };
...
>
  ...
  <span className="text-sm">Read Aloud</span>
      ...
      <SEO
        title="Documentaries - Movie & TV trailers"
        description="Watch the best documentary films free in HD on Movie & TV trailers."
        schema={schema}
        path="/Docs"
      />
      ...
      <div className="container mx-auto px-4 md:px-6 mt-10">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">Documentaries</h1>
            <p className="text-gray-400 text-sm">{UNIQUE_DOCUMENTARY.length} titles — Nature, History, Adventure & more</p>
          </div>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
          {UNIQUE_DOCUMENTARY.map((item) => (
