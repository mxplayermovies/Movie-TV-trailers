...
// import { UNIQUE_HINDI_DUBBED, getDetails, getImageUrl } from '../../../services/tmdb';
...
// export default function HindiDubbedDetail({ item, recommendations, ogImage }: Props) {
//   const router = useRouter();
//   const [loading, setLoading] = useState(false);
//   const [isShareOpen, setIsShareOpen] = useState(false);
//   const [copiedLink, setCopiedLink] = useState(false);

//   const title = item.title || item.name || 'Hindi Dubbed Movie';
...
//   useEffect(() => {
//     if (title) {
//       voiceManager.speak(`Now viewing ${title}. Click the speaker icon to know about the movie. Please select the languages from the player settings options.`);
//     }
//   }, [title]);
...
//   // Movie schema (Hindi dubbed movies are still movies)
//   const movieSchema = {
...
//     "contentRating": "PG-13", // Adjust if needed
...
//     const recommendations = UNIQUE_HINDI_DUBBED
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  try {
    ...
    const imagePath = sanitizedItem.backdrop_path || sanitizedItem.poster_path;
    if (!imagePath) {
      console.error(`hindi-dubbed "${sanitizedItem.title}" (id: ${id}) has no image.`);
      return { notFound: true };
    }
