...
//   // Extract bg music ID from post if available (post has yt_bg_music_id)
//   const bgMusicId = post?.yt_bg_music_id || null;

//   // Set background music for this blog post (if any)
//   useEffect(() => {
...
//       const pageText = getPageText();
//       if (pageText) {
//         voiceManager.speak(pageText, true, true); // ADDED third true
//       } else {
//         voiceManager.speak(`Now reading: ${post.title}`, true, true); // ADDED third true
//       }
//     }, 3000);
//     return () => clearTimeout(timer);
//   }, [post, getPageText]);
...
import YouTubePlayer from '../../components/YouTubePlayer';
import ShareButtons from '../../components/ShareButtons';
import { Calendar, User, Tag, ArrowLeft, Volume2, VolumeX } from 'lucide-react';
import Link from 'next/link';
...
export default function BlogPostPage({ post }: Props) {
  ...
  const [isVoiceReady, setIsVoiceReady] = useState(false); // Track when voiceManager is loaded
  ...
  // Load voiceManager into ref and mark as ready
  useEffect(() => {
    let mounted = true;
  ...
  // Auto‑speak after delay (only if not muted and voice is ready)
  useEffect(() => {
    if (!post || !isVoiceReady) return;
    const timer = setTimeout(() => {
      if (isMuted) return;
      const pageText = getPageText();
      if (pageText) {
        voiceRef.current?.speak(pageText, true, true);
      } else {
        voiceRef.current?.speak(`Now reading: ${post.title}`, true, true);
      }
    }, 3000);
    return () => clearTimeout(timer);
  }, [post, getPageText, isMuted, isVoiceReady]);
