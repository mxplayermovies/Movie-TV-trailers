...
// We update our content regularly to ensure you have the latest and greatest movie trailers and information every 24 hours. Watch our Latest movie tv show, sports, live tv, hindi dubbed and documentary movies library and enjoy the best free streaming experience.
...
export default function BlogIndex({ posts, buildDateString }: Props) {
  ...
  useEffect(() => {
    // Start continuous background music immediately
    voiceManager.playBackgroundMusic(DEFAULT_BG_MUSIC_IDS.blog);

    const timer = setTimeout(() => {
      const text = getPageText() || 'Blog page. Read the latest news, reviews, and guides about movies, TV shows, live sports, and more.';
  ...
  const readPageContent = () => {
    ...
    if (text) {
      ...
      voiceManager.speak(
        'Blog page. Read the latest news, reviews, and guides about movies, TV shows, live sports, and more.',
        true,
        true
      );
    }
  ...
  const loadMore = () => setVisibleCount(prev => Math.min(prev + 15, totalItems));
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a]">
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-4xl md:text-5xl font-bold">Blog</h1>
            <p className="page-text hidden">
              We have {posts.length} blog posts. Updated as on {buildDateString}. We update our content regularly to ensure you have the latest and greatest movie trailers and information every 24 hours. Watch our Latest movie tv show, sports, live tv, hindi dubbed and documentary movies library and enjoy the best free streaming experience.
            </p>
              ...
              return (
                <article key={post.id} className="bg-white dark:bg-[#1e293b] rounded-xl shadow-lg overflow-hidden border border-slate-200 dark:border-white/5 hover:shadow-xl transition">
                  <Link href={`/blog/${post.slug}`}>
                    <div className="cursor-pointer">
                      <div className="aspect-video relative overflow-hidden">
                        <img src={post.image} alt={post.title} className="w-full h-full object-cover hover:scale-105 transition duration-300" loading="lazy" />
                        {post.videoId && (
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          {post.tags && post.tags.length > 0 && (
                            <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-1 rounded-full">{post.tags[0]}</span>
                          )}
                          {post.tags.slice(0, 2).map((tag) => (
