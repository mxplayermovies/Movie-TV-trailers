...
// import { UNIQUE_SPORTS, getDetails, getImageUrl } from '../../../services/tmdb';
...
// export default function SportsDetail({ item, recommendations, ogImage }: Props) {
...
//     "image": ogImage,
//     "url": canonicalUrl,
//     "startDate": item.release_date || new Date().toISOString(),
//     "location": {
//       "@type": "Place",
//       "name": "Online Stream",
//       "address": {
...
//     <>
//       <Head>
//         <title>{title} - Watch Live Sports | Movie & TV Trailers</title>
...
//         <meta property="og:title" content={`${title} - Watch Live Sports`} />
...
//         <meta name="twitter:title" content={`${title} - Watch Live Sports`} />
...
//     const recommendations = UNIQUE_SPORTS
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  try {
    ...
    const imagePath = sanitizedItem.backdrop_path || sanitizedItem.poster_path;
    if (!imagePath) {
      console.error(`Sports item "${sanitizedItem.title}" (id: ${id}) has no image.`);
      return { notFound: true };
    }
