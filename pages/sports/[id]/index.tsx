...
// import { UNIQUE_SPORTS, getDetails } from '../../../services/tmdb';
...
// export default function SportsDetail({ item, recommendations }: Props) {
//   const router = useRouter();
//   const [loading, setLoading] = useState(false);

//   const title = item.title || item.name || 'Live Sports';
//   const ytId = item.yt_id;

//   useEffect(() => {
//     if (title) {
//       voiceManager.speak(`Now viewing ${title}. Click the speaker icon to learn about the sporting live action.`);
//     }
//   }, [title]);
...
//       year: 'numeric',
//     });
//     const text = `${title} for ${formattedDate}. ${item.overview || ''}...`;
//     voiceManager.speak(text, true);
//   };

//   const handlePlay = () => {
//     setLoading(true);
//     router.push(`/watch/${item.id}?type=sports`);
//   };
...
//     <>
//       <Head>
//         <title>{title} - Watch Live Sports</title>
...
//         <meta property="og:url" content={`https://movie-tv-trailers.vercel.app/sports/${item.id}`} />
...
//           <Recommendations items={recommendations} basePath="/sports" title="More Sports" />
//         </main>
//         <Footer />
//       </div>
//     </>
...
//   const paths = UNIQUE_SPORTS.map((item) => ({
...
//   try {
//     const item = await getDetails('sports', id);
//     const sanitizedItem = sanitizeMediaItem(item);

//     const allItems = UNIQUE_SPORTS.map(sanitizeMediaItem);
...
//   const title = item.title || item.name || 'Live Sports';
//   const ytId = item.yt_id;
//   const shareUrl = `${BASE_URL}/sports/${item.id}`;
...
export default function SportsDetail({ item, recommendations }: Props) {
  ...
  useEffect(() => {
    if (title) voiceManager.speak(`Now viewing ${title}. Click the speaker icon to learn about the sporting live action.`);
  }, [title]);

  const readDetails = () => {
    const formattedDate = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
    voiceManager.speak(`${title} for ${formattedDate}. ${item.overview || ''}...`, true);
  };

  const handlePlay = () => { setLoading(true); router.push(`/watch/${item.id}?type=sports`); };
      ...
      <Head>
        ...
        {ytId && (
          <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
            ...
            uploadDate: item.release_date || new Date().toISOString().split('T')[0],
            duration: 'PT0H0M', contentUrl: youtubeWatchUrl, embedUrl: youtubeEmbedUrl,
          })}} />
        )}
      </Head>
      <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a]">
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="flex justify-end mb-4">
            <button onClick={readDetails} className="p-2 bg-blue-500/20 text-blue-400 rounded-full hover:bg-blue-500 hover:text-white transition"><Volume2 size={20} /></button>
          </div>
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold mb-4">{title}</h1>
              ...
              <button onClick={handlePlay} disabled={loading} className="flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg transition disabled:opacity-50">
                <Play size={20} />{loading ? 'Loading...' : 'Watch Live'}
              </button>
              <ShareButtons url={shareUrl} title={title} description={description} />
            </div>
            <div><YouTubePlayer videoId={ytId} title={title} autoplay loop /></div>
          </div>
          <Recommendations items={recommendations} basePath="/sports" title="More Sports" />
        </main>
        <Footer />
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = UNIQUE_SPORTS.map((item) => ({ params: { id: String(item.id) } }));
  return { paths, fallback: 'blocking' };
};
