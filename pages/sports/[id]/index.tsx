// // pages/sports/[id]/index.tsx
// import React, { useEffect, useState, useCallback, useRef } from 'react';
// import Head from 'next/head';
// import { GetStaticPaths, GetStaticProps } from 'next';
// import { useRouter } from 'next/router';
// import { UNIQUE_SPORTS, getImageUrl } from '../../../services/tmdb';
...
//   const title = item.title || item.name || 'Sports';
//   const ytId = item.yt_id;
//   const canonicalUrl = `${BASE_URL}/sports/${item.id}`;
...
//                 </div>
//                 <button onClick={() => { setLoading(true); router.push(`/watch/${item.id}?type=sports`); }} disabled={loading} className="flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg transition disabled:opacity-50">
//                   <Play size={20} />{loading ? 'Loading…' : 'Play Now'}
//                 </button>
//               </div>
//             </div>
//           </div>
//           <div className="mt-12"><Recommendations items={recommendations} basePath="/sports" title="More Sports" /></div>
//         </main>
//         <Footer />
//       </div>
//     </>
...
//   paths: UNIQUE_SPORTS.map((item) => ({ params: { id: String(item.id) } })),
//   fallback: 'blocking',
// });
...
//   try {
//     const movie = UNIQUE_SPORTS.find((m) => String(m.id) === id);
...
//     const recommendations = UNIQUE_SPORTS.filter((m) => String(m.id) !== id).slice(0, 6).map((m) => sanitizeMediaItem(m));
//     return { props: { item: sanitizedItem, recommendations, ogImage }, revalidate: 3600 };
//   } catch (error) {
//     console.error(`Error in getStaticProps for sports ${id}:`, error);
...
//   // Set background music for this sports item (or null if none)
//   useEffect(() => {
...
//     import('../../../lib/core/VoiceManager').then(({ voiceManager }) => {
//       if (cancelled) return; voiceRef.current = voiceManager;
//       timer = setTimeout(() => { if (cancelled) return; try { voiceManager.speak(getPageText() || `Now viewing ${title}.`, true, true); } catch {} }, 1500); // ADDED third true
//     }).catch(() => {});
//     return () => { cancelled = true; clearTimeout(timer); };
//   }, [title, getPageText]);

//   const readDetails = useCallback(() => {
//     try { voiceRef.current?.speak(getPageText() || `Now viewing ${title}.`, true, true); } catch {} // ADDED third true
//   }, [getPageText, title]);
    ...
    import('../../../lib/core/VoiceManager')
      .then(({ voiceManager }) => {
        ...
        timer = setTimeout(() => {
          ...
          try { voiceManager.speak(getPageText() || `Now viewing ${title}.`, true, true); } catch {}
        }, 1500);
      })
      .catch(() => {});
    return () => {
