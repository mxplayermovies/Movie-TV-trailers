...
//   const router = useRouter();
//   const [loading, setLoading] = useState(false);
//   const voiceRef = useRef<{ speak: (text: string, force?: boolean, withMusic?: boolean) => void } | null>(null);

//   const title = item.title || item.name || 'Movie';
//   const ytId = item.yt_id;
//   const bgMusicId = item.yt_bg_music_id || null; // only if present

//   // Set background music for this movie (or null if none)
//   useEffect(() => {
...
//           if (cancelled) return;
//           try {
//             // withMusic = true (default), so if bgMusicId is set, music plays
...
//   const readDetails = useCallback(() => {
//     try {
//       voiceRef.current?.speak(getPageText() || `Now viewing ${title}.`, true, true);
//     } catch {
//       /* ignore */
//     }
//   }, [getPageText, title]);
...
//         {youtubeEmbedUrl && (
//           <>
//             <meta property="og:video:url" content={youtubeEmbedUrl} />
...
export default function MovieDetail({ item, recommendations, ogImage }: Props) {
  ...
  const voiceRef = useRef<{
    ...
    cancelSpeech: () => void;
    setMusicMuted: (muted: boolean) => void; // 🆕 ADDED
  } | null>(null);
  ...
  // 🆕 EFFECT: Handle mute changes – cancel speech and mute/unmute background music
  useEffect(() => {
    if (voiceRef.current) {
      if (isMuted) {
        voiceRef.current.cancelSpeech();
