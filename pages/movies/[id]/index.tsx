...
// ─── DO NOT import voiceManager at the top level ──────────────────────────────
// It uses window.speechSynthesis which does not exist in Node.js (server-side).
// That static import was crashing Next.js SSR and causing Google to get a 5xx.
// It is loaded dynamically inside useEffect (client-only) below.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function MovieDetail({ item, recommendations, ogImage }: Props) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  // voiceRef is only populated on the client, never on the server
  ...
  // Wrap formatDurationToISO so a bad duration value never crashes the page
  let isoDuration: string | undefined;
  try {
    isoDuration = item.duration ? formatDurationToISO(item.duration) : undefined;
  } catch {
  ...
  // Safe DOM text reader — only ever called from useEffect or click handlers
  // (both client-only), but we guard anyway
  const getPageText = useCallback((): string => {
    if (typeof document === 'undefined') return '';
    try {
      const titleEl = document.querySelector('h1');
  ...
  // ─── THE FIX THAT STOPS THE 5xx ──────────────────────────────────────────
  // Dynamic import means voiceManager only loads in the browser.
  // The server never sees it, so Next.js SSR completes successfully,
  // Google can fetch and index the page, and TTS still auto-plays for users.
  // ─────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    let cancelled = false;
    ...
    import('../../../lib/core/VoiceManager')
      .then(({ voiceManager }) => {
        ...
        // Auto-speak after DOM has painted — same 1500ms as before
        timer = setTimeout(() => {
          if (cancelled) return;
          try {
            const text = getPageText() || `Now viewing ${title}.`;
            ...
            // speechSynthesis unavailable in this browser — ignore
          }
        }, 1500);
      })
      .catch(() => {
        // VoiceManager failed to load — TTS is non-critical, ignore
      });
  ...
  // Manual read-aloud button handler
  const readDetails = useCallback(() => {
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8 max-w-7xl mx-auto">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-4">
                {/* .movie-overview is used by getPageText() to grab this text for TTS */}
                <p className="movie-overview text-gray-600 dark:text-gray-300 mb-4">
                  {item.overview}
                </p>
