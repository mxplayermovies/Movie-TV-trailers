// // pages/tv/[id]/index.tsx
// import React, { useEffect, useState, useCallback, useRef } from 'react';
// import Head from 'next/head';
// import { GetStaticPaths, GetStaticProps } from 'next';
// import { useRouter } from 'next/router';
// import { UNIQUE_TV_SHOWS, getImageUrl } from '../../../services/tmdb';
// import { MediaItem } from '../../../types';
// import Header from '../../../components/Header';
// import Footer from '../../../components/Footer';
// import YouTubePlayer from '../../../components/YouTubePlayer';
// // ✅ FIX: no top-level voiceManager import — dynamic import inside useEffect only
...
// export default function TVShowDetail({ item, recommendations, ogImage }: Props) {
...
//   const canonicalUrl = `${BASE_URL}/tv/${item.id}`;
...
//       const o = document.querySelector('.tv-overview')?.textContent || '';
//       return t ? `${t}. ${o}` : o;
//     } catch { return ''; }
//   }, []);
...
//                   <button onClick={readDetails} className="p-2 bg-blue-500/20 text-blue-400 rounded-full hover:bg-blue-500 hover:text-white transition" title="Read aloud" aria-label="Read details aloud">
//                     <Volume2 size={20} />
//                   </button>
//                 </div>
//               </div>
...
//               <div className="mt-4">
//                 <p className="tv-overview text-gray-600 dark:text-gray-300 mb-4">{item.overview}</p>
...
//           <div className="mt-12">
//             <Recommendations items={recommendations} basePath="/tv" title="More TV Shows" />
//           </div>
//         </main>
//         <Footer />
//       </div>
...
//   paths: UNIQUE_TV_SHOWS.map((item) => ({ params: { id: String(item.id) } })),
//   fallback: 'blocking',
// });
...
//   try {
//     const show = UNIQUE_TV_SHOWS.find((m) => String(m.id) === id);
//     if (!show) return { notFound: true };

//     const sanitizedItem = sanitizeMediaItem(show);
...
//     const recommendations = UNIQUE_TV_SHOWS
...
//   } catch (error) {
//     console.error(`Error in getStaticProps for tv ${id}:`, error);
...
//   const router = useRouter();
//   const [loading, setLoading] = useState(false);
//   const voiceRef = useRef<{ speak: (text: string, force?: boolean, withMusic?: boolean) => void } | null>(null); // UPDATED type

//   const title = item.title || item.name || 'TV Show';
//   const ytId = item.yt_id;
//   const bgMusicId = item.yt_bg_music_id || null; // ADDED

//   // Set background music for this TV show (or null if none)
//   useEffect(() => {
//     musicController.setCurrentMusicId(bgMusicId);
//     return () => musicController.setCurrentMusicId(null);
//   }, [bgMusicId]);
...
//         if (cancelled) return;
//         try {
//           voiceManager.speak(getPageText() || `Now viewing ${title}.`, true, true); // ADDED third true
//         } catch { /* ignore */ }
//       }, 1500);
//     }).catch(() => {});
//     return () => { cancelled = true; clearTimeout(timer); };
...
//   const readDetails = useCallback(() => {
//     try {
//       voiceRef.current?.speak(getPageText() || `Now viewing ${title}.`, true, true); // ADDED third true
//     } catch { /* ignore */ }
//   }, [getPageText, title]);
...
import Footer from '../../../components/Footer';
import YouTubePlayer from '../../../components/YouTubePlayer';
import { Play, Volume2, VolumeX } from 'lucide-react'; // Updated import
import { sanitizeMediaItem } from '../../../lib/core/sanitize';
import Recommendations from '../../../components/Recommendations';
import { formatDurationToISO } from '../../../lib/utils/duration';
...
export default function TVShowDetail({ item, recommendations, ogImage }: Props) {
  ...
  const title = item.title || item.name || 'TV Show';
  const ytId = item.yt_id;
  const bgMusicId = item.yt_bg_music_id || null;
    ...
    import('../../../lib/core/VoiceManager')
      .then(({ voiceManager }) => {
        ...
        timer = setTimeout(() => {
          ...
          try {
            voiceManager.speak(getPageText() || `Now viewing ${title}.`, true, true);
          } catch { /* ignore */ }
        }, 1500);
      })
      .catch(() => {});
