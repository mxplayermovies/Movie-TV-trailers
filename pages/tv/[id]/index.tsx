...
export default function TvShowDetail({ item, recommendations }: Props) {
  ...
  useEffect(() => {
    if (title) voiceManager.speak(`Now viewing ${title}. Click the speaker icon to learn more about the TV series.`);
  }, [title]);

  const readDetails = () => voiceManager.speak(`${title}. ${item.overview || ''}...`, true);
  const handlePlay = () => { setLoading(true); router.push(`/watch/${item.id}?type=tv`); };
      ...
      <Head>
        ...
        {ytId && (
          <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
            ...
            uploadDate: item.release_date || new Date().toISOString().split('T')[0],
            duration: item.duration || 'PT45M', contentUrl: youtubeWatchUrl, embedUrl: youtubeEmbedUrl,
          })}} />
        )}
      </Head>
      <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a]">
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = UNIQUE_TV_SHOWS.map((item) => ({ params: { id: String(item.id) } }));
  return { paths, fallback: 'blocking' };
};
