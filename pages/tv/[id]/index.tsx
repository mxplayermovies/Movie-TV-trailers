...
// import { UNIQUE_TV_SHOWS, getDetails, getImageUrl } from '../../../services/tmdb';
...
//         "uploadDate": item.first_air_date || item.release_date || new Date().toISOString().split('T')[0],
...
//     const allItems = UNIQUE_TV_SHOWS.map(sanitizeMediaItem);
...
//   const id = params?.id as string;
//   try {
//     const movie = UNIQUE_TV_SHOWS.find((m) => String(m.id) === id);
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  try {
    ...
    const imagePath = sanitizedItem.backdrop_path || sanitizedItem.poster_path;
    if (!imagePath) {
      console.error(`tv "${sanitizedItem.title}" (id: ${id}) has no image.`);
      return { notFound: true };
    }
