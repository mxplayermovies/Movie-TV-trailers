...
// ✅ NO voiceManager import here at all.
// VoiceManager is loaded lazily inside useEffect (client-only) below.
...
export default function App({ Component, pageProps }: AppProps) {
  ...
  useEffect(() => {
    ...
    } catch { /* localStorage blocked in some private browsers */ }
  }, []);

  // ── VoiceManager unlock (client-only, lazy) ───────────────────────────
  useEffect(() => {
    import('../lib/core/VoiceManager')
      .then(({ voiceManager }) => voiceManager.unlock())
      .catch(() => { /* voice not critical */ });
  }, []);
  ...
  return (
    <>
      <ThemeContext.Provider value={{ isDarkMode, toggleTheme }}>
        ...
        {/* ── Ad network scripts ─────────────────────────────────────────── */}
        {/*
          These load nap5k.com and gizokraijaw.net which in turn load trackers
          from rtmark.net and jhnwr.com. Those console errors you see are
          NORMAL — they are the ad networks doing their own telemetry.
          They do NOT cause 5xx errors. They are browser-side only.
        */}
        <Script id="ad-zone-1" strategy="afterInteractive">
          {`
            (function(){
              var s = document.createElement('script');
              s.dataset.zone = '10641698';
        ...
        <Script id="ad-zone-2" strategy="afterInteractive">
          {`
            (function(){
              var s = document.createElement('script');
              s.dataset.zone = '10641706';
        ...
        {/* ── Clicky Analytics ──────────────────────────────────────────── */}
        {/* ✅ Fixed: use https:// not protocol-relative // to avoid mixed-content errors */}
        <Script
        ...
        {/* ── Right-click warning toast ─────────────────────────────────── */}
        <div
          className={`fixed top-24 left-1/2 -translate-x-1/2 z-[100] transition-all duration-300 pointer-events-none ${
