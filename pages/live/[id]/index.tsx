...
// export default function LiveTvDetail({ item, recommendations }: Props) {
...
//         <meta property="og:url" content={`https://movie-tv-trailers.vercel.app/live/${item.id}`} />
...
export default function LiveTvDetail({ item, recommendations }: Props) {
  ...
  useEffect(() => {
    if (title) voiceManager.speak(`Now viewing ${title}. Click the speaker icon to learn about the latest updated news.`);
  }, [title]);

  const readDetails = () => voiceManager.speak(`${title}. ${item.overview || ''}...`, true);
  const handlePlay = () => { setLoading(true); router.push(`/watch/${item.id}?type=tv_live`); };
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = UNIQUE_TV_LIVE.map((item) => ({ params: { id: String(item.id) } }));
  return { paths, fallback: 'blocking' };
};
