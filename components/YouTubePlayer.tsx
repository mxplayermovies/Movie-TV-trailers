...
//             style={{
//                   filter: 'brightness(1.1) contrast(1.08) saturate(1.1) sepia(0) hue-rotate(2deg)'
//                               // filter: 'brightness(1.1) contrast(1.4) saturate(1.1)'
        ...
        playerInstanceRef.current = new window.YT.Player(playerRef.current, {
          ...
          events: {
            onReady: (event: any) => {
              ...
              // Get the thumbnail directly from the YouTube IFrame API player instance.
              // getVideoData() returns { video_id, author, title, video_quality, ... }
              // We build the thumbnail from the confirmed video_id the player is actually using.
              try {
                const videoData = event.target.getVideoData()
                const confirmedId = videoData?.video_id || videoId
                // maxresdefault first, hqdefault as the reliable fallback
                const thumbUrl = `https://i.ytimg.com/vi/${confirmedId}/hqdefault.jpg`
                if (onThumbnailReady) onThumbnailReady(thumbUrl)
              } catch (e) {
                // fallback using the prop videoId directly
