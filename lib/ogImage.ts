 ...
 * lib/ogImage.ts
 *
 * Uses wsrv.nl (images.weserv.nl) — a free, open-source image proxy CDN
 * that requires ZERO signup, ZERO API keys, and is trusted by Facebook
 * and Twitter scrapers.
 *
 * wsrv.nl fetches any public image and serves it from their CDN.
 * Facebook and Twitter can always load images from images.weserv.nl
 */
...
function buildSocialImageUrl(path: string | null | undefined): string {
  ...
  // Local assets on your own domain — always accessible to scrapers
  if (
    path.startsWith('/images/') ||
    path === '/18only.png' ||
    path === '/logo.png' ||
  ...
  if (path.startsWith('http')) {
    ...
    // TMDB relative path like /rktDFPbfHfUbArZ6OOOKsXcv0Bs.jpg
    const clean = path.startsWith('/') ? path : `/${path}`;
    fullUrl = `https://image.tmdb.org/t/p/w1280${clean}`;
  }

  // wsrv.nl proxy — free, no account needed, works with all social scrapers
  // strips the https:// prefix as required by wsrv.nl API
  const stripped = fullUrl.replace(/^https?:\/\//, '');
  return `https://images.weserv.nl/?url=${encodeURIComponent(stripped)}&w=1200&h=630&fit=cover&output=jpg&q=80`;
}
